...
async function main() {
    ...
    // Driver users
    const driverUsers = await Promise.all(Array.from({ length: 20 }, (_, i) => {
    ...
    const fuelTypes = ['DIESEL', 'CNG', 'CNG', 'DIESEL', 'ELECTRIC'];
    const buses = await Promise.all(Array.from({ length: 30 }, (_, i) => {
        const modelIdx = i % 5;
    ...
    const drivers = await Promise.all(driverUsers.map((user, i) => prisma.driver.create({
        data: {
            userId: user.id,
    ...
    const stops = await Promise.all(stopData.map((s) => prisma.busStop.create({
        data: {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        sDate.setDate(sDate.getDate() + (i - 5));
        await prisma.maintenanceRecord.create({
            data: {
                busId: buses[i % buses.length].id,
                type: ['ROUTINE', 'INSPECTION', 'ROUTINE', 'EMERGENCY', 'OVERHAUL'][i % 5],
    ...
    for (let i = 0; i < 7; i++) {
        ...
        for (let j = 0; j < Math.min(drivers.length, 15); j++) {
            const checkIn = new Date(aDate);
            checkIn.setHours(6 + (j % 3) * 8, Math.floor(Math.random() * 30));
    ...
    // ============ NOTIFICATIONS ============
    const notifTypes = ['BUS_DELAY', 'MAINTENANCE', 'SHIFT_CHANGE', 'SYSTEM', 'ROUTE_CHANGE'];
    for (let i = 0; i < 10; i++) {
